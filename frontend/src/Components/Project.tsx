  ...
  return (
    <div className="card_background text-white rounded-md font-SpotifyMedium h-full pb-10">
      ...
      >
        <div className="flex flex-row gap-x-4 mb-4">
          <div className="flex flex-col justify-center">
            <h3 className="text-white text-4xl pl-4 font-bold md:text-7xl">
              {" "}
              {project.title}{" "}
            </h3>
            <p className="text-white text-2xl SpotifyLightFont font-medium pl-4 pb-4 md:text-2xl">
              {project.date}
            </p>
          </div>
        </div>
      ...
      <div className="h-auto w-full flex flex-row flex-wrap gap-y-4 justify-between px-4 my-4 mt-[-100px]">
        <div className="flex flex-col w-[50%]">
          ...
          {project.techStack &&
            project.techStack.map((techStack, index) => (
              <div
                key={index}
                className="hover-effect2 flex flex-row rounded-md w-full h-auto min-w-[350px] gap-x-6 pl-2 md:pl-6 items-center SpotifyLightFont"
              >
                <h2 className="text-gray-400 w-2">{index + 1}</h2>
                <Image
                  className="rounded-md w-[35px] h-[35px] object-contain m-2"
                  src={`/images/techStack/${techStack}.png`}
                  alt="Tech Stack"
                  width={35}
                  height={35}
                />
                <h1 className="text-white text-xl SpotifyLightFont">
                  {techStack}
                </h1>
              </div>
            ))}
        ...
        <div className="flex flex-col sm:flex-row xl:flex-col rounded-md w-[600px] h-auto items-top gap-y-4 gap-x-10">
          {project.link &&
            <div>
                <a className="text-2xl">Link to Repository</a>
                <a href={project.link ? project.link : "/"}>
                <Image
                    className="rounded-sm w-[75px] h-[75px] object-contain"
                    width={75}
                    height={75}
          ...
          {project.hostedLink && <div>
            <a className="text-2xl">Link to live website</a>
            <a href={project.hostedLink ? project.hostedLink : "/"}>
              <Image
                className="rounded-sm w-[75px] h-[75px] object-contain"
                width={75}
                height={75}
        ...
        <div className="px-4 flex flex-col w-full">
          ...
          <div className="flex flex-row w-full overflow-y-hidden h-[220px] max-h-[240px] min-h-[220px] gap-x-2 overflow-x-auto horizontalScroller">
            {project.demoImages.map((aDemoImage, index) => (
              <ImageModal
                key={index}
                imageURL={aDemoImage.image}
                caption={aDemoImage.caption}
              />
            ))}
          </div>
        </div>
