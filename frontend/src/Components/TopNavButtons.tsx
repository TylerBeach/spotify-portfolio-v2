...
function TopNavButtons({title, imageURL}:{title: string | null, imageURL: string | undefined | null}) {
  ...
  return (
    <div
      className="right-0 sm:right-2 duration-700 fixed flex flex-row justify-between items-center top-0 top-buttons-width pl-4 h-auto py-4 pr-4 md:pr-6 text-white md:rounded-tl-md md:top-2 z-[40]"
      style={{
    ...
    >
      ...
      {showMenu && <div
        onClick={handleMenuClick}
        className="fixed top-0 left-0 bottom-0 right-0 w-[100vw] h-[100vh] bg-black opacity-40 z-[900]"
        ></div> }
